...
import { registerUser, resetAuth } from "../../store/authSlice"; // Import resetAuth
...
const Register = () => {
  ...
  useEffect(() => {
    console.log("Mount/Register Status:", status); // Debug initial status
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    console.log("Submitting with data:", userData); // Debug submitted data
    setErrors({ username: "", email: "", phoneNumber: "", password: "", general: "" });
    ...
    if (!validatePhone(userData.phoneNumber)) {
      setErrors({ ...errors, phoneNumber: "Invalid phone number format. It should be exactly 10 digits." });
      return;
    }
    ...
    dispatch(registerUser(userData))
      .unwrap()
      .catch((error) => {
        console.log("Registration Error:", error.response?.data); // Debug error response
        ...
        if (error.response?.status === 400) {
          if (errorMsg.includes("User already exists")) {
            ...
            setErrors({ ...errors, phoneNumber: errorMsg });
          } else {
  ...
  useEffect(() => {
    console.log("Status Changed to:", status); // Debug status changes
          ...
          <div>
            ...
            />
                        <button
                          ...
                          className="cursor-pointer absolute inset-y-0 flex items-center pr-4 mt-97 ml-88 text-gray-500 hover:text-gray-700 focus:outline-none"
                        >
                          {showPassword ? (
